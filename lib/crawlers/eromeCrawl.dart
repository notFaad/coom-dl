...
class EromeCrawler {
  Future<Map<String, dynamic>> init(
    ...
    try {
      Map<String, dynamic> siteContents =
          await _getSiteContent(url: url, scrape_logg: scrape_logg);
      return siteContents;
    } catch (e) {
  ...
  Future<Map<String, dynamic>> _getSiteContent(
      {required url, required scrape_logg}) async {
    dom.Document html;
    ...
    while (nextPage != null) {
      try {
        ...
        if (response.statusCode != 200) {
          scrape_logg({
            "status": "error",
            "m": "https://erome.com${nextPage.attributes['href']}"
          });
        } else {
          scrape_logg({
            "status": "scrap",
    ...
    if (contents_images.isEmpty && contents_vids.isEmpty) {
      ...
      for (int i = 0; i < contents_a.length; i++) {
        ...
        if (response.statusCode != 200) {
          scrape_logg(
              {"status": "fail", "m": "${contents_a[i].attributes['href']}"});
        } else {
          scrape_logg(
              {"status": "scrap", "m": "${contents_a[i].attributes['href']}"});
        }
        html = dom.Document.html(response.body);
        contents_images
